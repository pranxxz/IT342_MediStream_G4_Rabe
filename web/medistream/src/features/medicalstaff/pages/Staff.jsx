...
const Staff = () => {
  ...
  const handleAddStaffSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        role: newStaffData.role.toLowerCase(), // Backend role needs to be lowercase
        idNumber: 'MS' + Date.now().toString().slice(-6), // Autogenerated identifier
      };
  ...
  const handleEditStaffSubmit = async (e) => {
    ...
    try {
      ...
      // Map updated staff back to selectedStaff so view mode refreshes
      ...
      <div style={{ padding: "0 40px 40px" }}>
        ...
        {selectedStaff && (
          <div style={{
            ...
            right: 0,
            bottom: 0,
            background: 'rgba(68, 0, 13, 0.4)', // tint of brand maroon with transparency
          ...
          >
            <div style={{
              background: WHITE,
              borderRadius: 12,
              boxShadow: "0 8px 25px rgba(68,0,13,0.12)", border: `1px solid ${BORDER}`,
            }}>
              width: '90%',
            ...
            >
              ...
              {/* Header info */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 28, borderBottom: `1px solid ${ROW_DIV}`, paddingBottom: 20 }}>
                <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <h3 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: TEXT }}>{selectedStaff.name || 'Unknown Name'}</h3>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <RoleBadge role={selectedStaff.role} roleColor={getRoleColor(selectedStaff.role)} />
                    <span style={{
                      display: "inline-block", padding: "4px 14px", borderRadius: 20,
                      background: selectedStaff.status ? getStatusBgColor(selectedStaff.status) : '#f3f4f6', 
                      color: selectedStaff.status ? getStatusColor(selectedStaff.status) : '#6b7280', 
                      border: `1px solid ${selectedStaff.status ? getStatusColor(selectedStaff.status) : '#6b7280'}30`,
                ...
                <>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 14, fontWeight: 600, color: TEXT }}>#{selectedStaff.id || 'N/A'}</div>
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: 11, fontWeight: 800, color: M, letterSpacing: '0.5px', marginBottom: 4 }}>DEPARTMENT</label>
                        <div style={{ fontSize: 14, fontWeight: 600, color: TEXT }}>{selectedStaff.department || 'General Medicine'}</div>
                      </div>
                    </div>
                    ...
                    <div style={{ borderTop: `1px solid ${ROW_DIV}`, paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                      <div>
                  ...
                  <div style={{ 
                    ...
                    <button 
                      ...
                      style={{
                        gridColumn: isAdmin() ? 'span 2' : 'span 1',
                        padding: '12px 16px',
                        borderRadius: 8,
                        border: `1.5px solid ${M}`,
                        background: M,
